...
use self::fraction::{BigFraction, Fraction, Sign};
...
impl Float {
    ...
    fn base_decode(
        ...
        if nan {
            return Ok(MaybeBigFraction::from(Fraction::nan()));
        } else if inf {
...
impl MaybeBigFraction {
    pub fn new(mut num: Mint, mut den: Mint) -> MaybeBigFraction {
        if num.is_i() && den.is_i() {
            ...
            let n = if num.is_i() {
                BigUint::from(num.get_i())
            } else {
                num.take_b()
            };
            let d = if den.is_i() {
                BigUint::from(den.get_i())
            } else {
                den.take_b()
            };
...
impl FloatValue {
    fn new(value: MaybeBigFraction) -> FloatValue {
        FloatValue {
            style: 0,
            alias: None,
            value: value,
...
impl From<MaybeBigFraction> for FloatValue {
    fn from(f: MaybeBigFraction) -> FloatValue {
        FloatValue::new(f)
    }
}
...
impl From<BigFraction> for FloatValue {
    fn from(f: BigFraction) -> FloatValue {
        FloatValue::new(MaybeBigFraction::Big(f))
    }
}

impl From<Fraction> for FloatValue {
    fn from(f: Fraction) -> FloatValue {
        FloatValue::new(MaybeBigFraction::Fra(f))
    }
}
    ...
    macro_rules! decoded_is_frac {
        ($coder:expr, $str:expr, ($num:expr, $den:expr)) => {{
            if let Ok(tagged) = $coder.decode(true, &$str.to_string().into_bytes()) {
                ...
                assert_eq!(
                    val,
                    BigFraction::new(BigUint::from($num as u64), BigUint::from($den as u64))
                );
            } else {
                assert!(false)
            }
