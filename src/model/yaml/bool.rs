...
impl Bool {
    ...
    fn base_decode(&self, explicit: bool, value: &[u8], yaml_11: bool) -> Result<bool, ()> {
        ...
        match value.get(ptr).map(|b| *b) {
            ...
            Some(b'T') => match value.get(ptr + 1..ptr + 4) {
                Some(s) => {
                    if s == "rue".as_bytes() || s == "RUE".as_bytes() {
                        ptr += 4;
                        found_val = 3;
                    }
                }
            ...
            Some(b'F') => match value.get(ptr + 1..ptr + 5) {
                Some(s) => {
                    if s == "alse".as_bytes() || s == "ALSE".as_bytes() {
                        ptr += 5;
                        found_val = 1;
                    }
                }
            ...
            Some(b'o') => match value.get(ptr + 1).map(|b| *b) {
                ...
                Some(b'f') => {
                    if yaml_11 {
                        if let Some(b'f') = value.get(ptr + 2).map(|b| *b) {
                            ptr += 3;
                            found_val = 1;
                        }
                    }
            ...
            Some(b'y') => match value.get(ptr + 1).map(|b| *b) {
                Some(b'e') => {
                    if yaml_11 {
                        if let Some(b's') = value.get(ptr + 2).map(|b| *b) {
                            ptr += 3;
                            found_val = 3;
                        }
                    }
...
impl Model for Bool {
    ...
    fn decode(&self, explicit: bool, value: &[u8]) -> Result<TaggedValue, ()> {
        Ok(TaggedValue::from(BoolValue::from(
            self.base_decode(explicit, value, false)?,
        )))
    }

    fn decode11(&self, explicit: bool, value: &[u8]) -> Result<TaggedValue, ()> {
        Ok(TaggedValue::from(BoolValue::from(
            self.base_decode(explicit, value, true)?,
        )))
    }
}
...
impl BoolValue {
    pub fn new(value: bool, styles: CommonStyles, alias: Option<Cow<'static, str>>) -> BoolValue {
        BoolValue {
...
impl From<bool> for BoolValue {
    fn from(val: bool) -> BoolValue {
        BoolValue::new(val, CommonStyles::default(), None)
    }
}
...
mod tests {
    ...
    use crate::model::{Renderer, Tagged, TaggedValue};
    ...
    fn decode11() {
        ...
        for i in 0..options.len() {
            ...
            assert!(tagged.is_ok(), "Cannot decode '{}'", options[i]);
    ...
    fn decode() {
        ...
        let options = ["true", "True", "TRUE", "false", "False", "FALSE"];

        let results = [true, true, true, false, false, false];
    ...
    fn encode() {
        ...
        if let Ok(rope) = bool.encode(
            &renderer,
            TaggedValue::from(BoolValue::from(true)),
            &mut iter::empty(),
        ) {
        ...
        if let Ok(rope) = bool.encode(
            &renderer,
            TaggedValue::from(BoolValue::from(false)),
            &mut iter::empty(),
        ) {
            let encode = rope.render(&renderer);
