    ...
    fn base_decode(
        ...
        if !explicit
            && !match value.get(0).map(|b| *b) {
                Some(b'+') | Some(b'-') | Some(b'0'...b'9') => true,
                _ => false,
            }
        {
            return Err(());
        ...
        if state & STATE_SIGN_N == STATE_SIGN_N {
            val = -val;
        };
...
impl Model for Int {
    ...
    fn decode(&self, explicit: bool, value: &[u8]) -> Result<TaggedValue, ()> {
        Ok(TaggedValue::from(IntValue::new(
            self.base_decode(explicit, value, false, false)?,
        )))
    }

    fn decode11(&self, explicit: bool, value: &[u8]) -> Result<TaggedValue, ()> {
        Ok(TaggedValue::from(IntValue::new(
            self.base_decode(explicit, value, true, true)?,
        )))
    }
}
...
mod tests {
    ...
    fn encode() {
        ...
        for i in 0..options.len() {
            if let Ok(rope) = int.encode(
                &renderer,
                TaggedValue::from(IntValue::from(options[i])),
                &mut iter::empty(),
            ) {
    ...
    fn decode() {
        ...
        let results = [
            15, 685230, 2472256, 685230, 685230, 685230, 685230, 685230, -685230, 15,
        ];
        ...
        for i in 0..options.len() {
            if let Ok(tagged) = int.decode(false, &options[i].to_string().into_bytes()) {
                ...
                assert!(false, format!("Could not decode {}", &options[i]))
            }
        }
    ...
    fn decode11() {
        ...
        let results = [
            15, 685230, 685230, 685230, 685230, 685230, 685230, 685230, 685230,
        ];
