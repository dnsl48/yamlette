...
use crate::model::{Rope, Schema, TaggedValue};

use crate::orchestra::performer::{Performer, PerformerId, Play};
use crate::orchestra::OrchError;
...
impl Coord {
    pub fn new(vol: usize, idx: usize, lvl: usize) -> Coord {
        Coord {
            vol: vol,
            idx: idx,
            lvl: lvl,
...
impl NodeList {
    pub unsafe fn as_list(&self) -> &[Node] {
        mem::transmute::<*const [Node], &[Node]>(self.0)
    }
}
...
impl Record {
    ...
    pub fn get_rope(&self) -> &Rope {
        self.play.as_ref().unwrap().get_rope()
    }

    pub fn mark_zip_ready(&mut self) {
        self.state |= RECORD_ZIP_READY;
    }
    ...
    pub fn take(&mut self) -> Play {
        ...
        if self.is_legato() && !self.is_chord() {
            panic!("the record is legato and not chord yet")
        }
    ...
    pub fn borrow(&mut self) -> Play {
        ...
        if !self.is_legato() {
            panic!("the record is not legato")
        }
    ...
    pub fn is_done(&self) -> bool {
        self.state & RECORD_DONE == RECORD_DONE
    }

    pub fn is_legato(&self) -> bool {
        self.state & RECORD_LEGATO == RECORD_LEGATO
    }

    pub fn is_chord(&self) -> bool {
        self.state & RECORD_CHORD == RECORD_CHORD
    }

    pub fn is_zipped(&self) -> bool {
        self.state & RECORD_ZIPPED == RECORD_ZIPPED
    }

    pub fn is_notready_or_zipped_or_borrowed(&self) -> bool {
        self.state & (RECORD_DONE | RECORD_ZIP_READY | RECORD_ZIPPED | RECORD_BORROWED)
            != RECORD_DONE | RECORD_ZIP_READY
    }

    pub fn is_notready_or_notchordlegato_or_borrowed(&self) -> bool {
        let flag = self.state
            & (RECORD_DONE | RECORD_ZIP_READY | RECORD_BORROWED | RECORD_LEGATO | RECORD_CHORD);

        return flag != RECORD_DONE | RECORD_ZIP_READY
            && flag != RECORD_DONE | RECORD_ZIP_READY | RECORD_LEGATO | RECORD_CHORD;
    }
}
...
impl Volume {
    ...
    pub fn init(&mut self, mut size: usize, prefer_bot_border: bool) {
        ...
        if self.styles & VOLUME_STYLE_DIR_TAGS == VOLUME_STYLE_DIR_TAGS {
            size += 1;
        }
        if self.styles & VOLUME_STYLE_TOP_BORDER == VOLUME_STYLE_TOP_BORDER {
            size += 1;
        }
        if self.styles & VOLUME_STYLE_BOT_BORDER == VOLUME_STYLE_BOT_BORDER {
            size += 1;
        }
    ...
    pub fn style(&mut self) -> Option<Gesture> {
        ...
        if (self.styles & VOLUME_STYLE_DIR_YAML == VOLUME_STYLE_DIR_YAML)
            && (self.styled & VOLUME_STYLE_DIR_YAML != VOLUME_STYLE_DIR_YAML)
        {
        ...
        if (self.styles & VOLUME_STYLE_DIR_TAGS == VOLUME_STYLE_DIR_TAGS)
            && (self.styled & VOLUME_STYLE_DIR_TAGS != VOLUME_STYLE_DIR_TAGS)
        {
        ...
        if (self.styles & VOLUME_STYLE_TOP_BORDER == VOLUME_STYLE_TOP_BORDER)
            && (self.styled & VOLUME_STYLE_TOP_BORDER != VOLUME_STYLE_TOP_BORDER)
        {
        ...
        if self.records.len() != self.size - 1 {
            return None;
        }

        if (self.styles & VOLUME_STYLE_BOT_BORDER == VOLUME_STYLE_BOT_BORDER)
            && (self.styled & VOLUME_STYLE_BOT_BORDER != VOLUME_STYLE_BOT_BORDER)
        {
    ...
    pub fn flatten(&mut self) -> Option<Gesture> {
        ...
        for &(record_idx, is_chord) in self.legatos.iter() {
            ...
            let level = {
                ...
                if record.is_notready_or_zipped_or_borrowed() {
                    continue;
                }
            ...
            for idx in record_idx + 1..self.records.len() {
                ...
                if record.is_notready_or_notchordlegato_or_borrowed() {
                    break;
                }
        ...
        if ready && self.real_records == self.size {
            ...
            let _old = mem::replace(&mut self.legatos, Vec::with_capacity(0));
        }
...
impl Conductor {
    pub fn run<S: Schema + Clone + 'static>(
        ...
        let handle = Builder::new()
            ...
            .spawn(move || {
                let (to_conductor, cin): (
                    SyncSender<(PerformerId, Play)>,
                    Receiver<(PerformerId, Play)>,
                ) = sync_channel(PERFORMERS_NUMBER);
    ...
    fn execute(mut self) -> Result<(), OrchError> {
        ...
        let mut performer_buffers: [*const [Node]; PERFORMERS_NUMBER] =
            [&[] as *const [Node]; PERFORMERS_NUMBER];
    ...
    fn play_to_volume(play: Play, volumes: &mut Vec<Volume>) {
        ...
        unsafe {
            volumes.get_unchecked_mut(vol).play(play);
        }
    }
    ...
    fn _execute(
        ...
        'main_loop: loop {
            ...
            if volumes.len() > 0 && !do_the_music {
                ...
                if flush {
                    ...
                    for vol in volumes.iter() {
                        music_length += vol.bytes_len;
                    }

                    music.reserve_exact(music_length);
                    unsafe {
                        music.set_len(music_length);
                    }
            ...
            if do_the_music {
                if self.msgs != 0 {
                    panic!("Someone still plays something")
                }
                ...
                if result.is_err() {
                    return Err(OrchError::Error(String::from(
                        "Could not send out the music",
                    )));
                }
                    ...
                    Message::Hint(hint) => match hint {
                        ...
                        Hint::BorderBot(print) => {
                            if print {
                                ...
                                volumes.last_mut().unwrap().styles |=
                                    VOLUME_STYLE_BOT_BORDER_EXPLICIT_NO;
                            }
                        }
    ...
    fn do_the_music<'a, 'b, 'c>(
        ...
        for vol in vols.iter() {
            ...
            'vol_loop: loop {
                if rec_cnt == vol.zero_level_nodes {
                    break;
                }
                if rec_ptr >= vol.size {
                    break;
                ...
                rope_idx = 0;
                loop {
                    let (size, new_index, done) = rope.unrope(
                        &mut performer_buffers[perf_idx],
                        &self.renderer,
                        rope_idx,
                        tho,
                    );
        ...
        for el in performer_buffers.iter_mut() {
            *el = &[];
        }
        ...
        'main_loop: loop {
                ...
                if result.is_err() {
                    match result {
                        Err(TrySendError::Disconnected(_)) => {
                            self.terminate();
                            return Err(OrchError::Error(format!(
                                "One of performers ({}) passed away",
                                idx
                            )));
                        }
            ...
            if self.msgs > 0 {
                let bval = match self.cin.recv() {
                    Ok((i, m)) => {
                        self.performers[i as usize]
                            .0
                            .send(gesture)
                            .or_else(|_| {
                                self.terminate();
                                return Err(OrchError::Error(String::from(
                                    "One of performers has gone",
                                )));
                            })
                            .and_then(|_| {
                                self.msgs += 1;
                    ...
                    Err(_) => {
                        return Err(OrchError::Error(
                            format!("The performers have gone").to_string(),
                        ))
                    }
                };
        ...
        if self.buff.is_some() {
            ...
            match self.cin.try_recv() {
                Err(TryRecvError::Disconnected) => {
                    Err(OrchError::Error("abandoned conductor".to_string()))
                }
                Ok((_, play)) => {
                    self.msgs -= 1;
                    Ok(Some(play))
