    ...
    D: Datum + 'static,
{
    pub fn new(schema: S) -> Savant<S, D> {
...

        {
            ...
            for th in th.iter().rev() {
                tag_handles.push((th.0.clone(), th.1.clone()));
            }
        }
    ...
    pub fn think(&mut self, block: Block<D>) -> Result<Option<Idea>, SageError> {
        match block.cargo {
            ...
            BlockType::Error(message, position) => {
                Ok(Some(Idea::ReadError(block.id, position, message)))
            }

            BlockType::Warning(message, position) => {
                Ok(Some(Idea::ReadWarning(block.id, position, message)))
            }
    ...
    fn read_block(&self, block: Block<D>) -> Result<Idea, SageError> {
        match block.cargo {
            ...
            BlockType::BlockMap(firstborn_id, anchor, tag) => match self.read_map(anchor, tag)? {
                Ok((tag, anchor)) => Ok(Idea::NodeDictionary(
                    block.id,
                    anchor,
                    tag,
                    Some(firstborn_id),
                ...
                Err((tag, anchor)) => {
                    Ok(Idea::NodeMetaMap(block.id, anchor, tag, Some(firstborn_id)))
                }
            },

            BlockType::Literal(marker) => Ok(Idea::NodeLiteral(
                block.id,
                None,
                self.read_literal(marker)?,
            )),

            BlockType::Byte(byte, amount) => Ok(Idea::NodeLiteral(
                block.id,
                None,
                self.read_byte(byte, amount)?,
            )),
    ...
    fn read_scalar(
        ...
        let node = if decoded.is_ok() {
            ...
            match model {
                Some((model, explicit)) => match self.decode(model, explicit, chunk) {
                    Ok(v) => v,
                ...
                None => {
                    ...
                    if let Some(m) = self.schema.get_metamodel() {
                        ...
                        meta = m.meta_init(anchor.clone(), self.resolve_tag(&tag), chunk);
                    }
    ...
    fn read_map(
        ...
        Result<(Cow<'static, str>, Option<String>), (Option<String>, Option<String>)>,
        SageError,
    > {
        let anchor: Option<String> = self.read_anchor(anchor)?;
        let tag: Option<String> = self.read_tag(tag)?;
    ...
    fn read_anchor(&self, anchor: Option<Marker>) -> Result<Option<String>, SageError> {
        if let Some(anchor) = anchor {
            ...
            let result = self
                .schema
                .get_model_literal()
                .bytes_to_string(chunk.as_slice());
    ...
    fn read_byte(&self, byte: u8, amount: usize) -> Result<String, SageError> {
        let literal = self
            .schema
            .get_model_literal()
            .bytes_to_string_times(&[byte], amount);
