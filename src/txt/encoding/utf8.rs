...
impl Unicode for UTF8 {
    ...
    unsafe fn to_unicode_ptr (&self, mut ptr: *const u8, len: usize) -> (u32, u8) {
        let (code, len) = if len > 0 {
            if *ptr & 0x80 == 0 {
                ...
                if _1   & 0xC0 == 0xC0 && _1   & 0x20 == 0 &&
                   *ptr & 0x80 == 0x80 && *ptr & 0x40 == 0
                {
                    ...
                    if _1   & 0xE0 == 0xE0 && _1   & 0x10 == 0 &&
                       _2   & 0x80 == 0x80 && _2   & 0x40 == 0 &&
                       *ptr & 0x80 == 0x80 && *ptr & 0x40 == 0
                    {
                        ( (((_1 as u32) ^ 0xE0) << 12) | (((_2 as u32) ^ 0x80) << 6) | ((*ptr as u32) ^ 0x80), 3 )
                    } else if len > 3 {
                        let _3 = *ptr as u8;
                        ptr = ptr.offset (1);

                        if _1   & 0xF0 == 0xF0 && _1   & 0x08 == 0 &&
                           _2   & 0x80 == 0x80 && _2   & 0x40 == 0 &&
                           _3   & 0x80 == 0x80 && _3   & 0x40 == 0 &&
                           *ptr & 0x80 == 0x80 && *ptr & 0x40 == 0
                        {
                            ( (((_1 as u32) ^ 0xF0) << 18) | (((_2 as u32) ^ 0x80) << 12) | (((_3 as u32) ^ 0x80) << 6) | ((*ptr as u32) ^ 0x80), 4 )
                        } else { ( 0xFFFD, 1 ) }
                    } else { ( 0xFFFD, 1 ) }
                } else { ( 0xFFFD, 1 ) }
            } else { ( 0xFFFD, 1 ) }
    ...
    fn to_unicode (&self, stream: &[u8]) -> (u32, u8) {
            ...
            && stream[1] & 0x40 == 0
        {
            ...
            return ( (((stream[0] as u32) ^ 0xC0) << 6) | ((stream[1] as u32) ^ 0x80), 2 )
            ...
            && stream[2] & 0x40 == 0
        {
            ...
            ( (((stream[0] as u32) ^ 0xE0) << 12) | (((stream[1] as u32) ^ 0x80) << 6) | ((stream[2] as u32) ^ 0x80), 3 )
            ...
            && stream[3] & 0x40 == 0
        {
            ...
            ( (((stream[0] as u32) ^ 0xF0) << 18) | (((stream[1] as u32) ^ 0x80) << 12) | (((stream[2] as u32) ^ 0x80) << 6) | ((stream[3] as u32) ^ 0x80), 4 )
...
mod tests {
    use super::UTF8;
    use txt::encoding::unicode::Unicode;
    ...
    fn to_unicode () {
        ...
        let src: [u8; 23] = [0xd1, 0x82, 0xd0, 0xb5, 0xd1, 0x81, 0xd1, 0x82, 0x20, 0xd1, 0x8e, 0xd0, 0xbd, 0xd0, 0xb8, 0xd0, 0xba, 0xd0, 0xbe, 0xd0, 0xb4, 0xd0, 0xb0];
        ...
        let src = [0xe7, 0xa7, 0x81, 0xe3, 0x81, 0xaf, 0xe7, 0xa7, 0x81, 0xe3, 0x81, 0x8c, 0xe6, 0x9b, 0xb8, 0xe3, 0x81, 0x84, 0xe3, 0x81, 0xa6, 0xe3, 0x81,
                   0x84, 0xe3, 0x82, 0x8b, 0xe3, 0x81, 0xae, 0xe3, 0x81, 0x8b, 0xe5, 0x88, 0x86, 0xe3, 0x81, 0x8b, 0xe3, 0x82, 0x8a, 0xe3, 0x81, 0xbe, 0xe3,
                   0x81, 0x9b, 0xe3, 0x82, 0x93];
        ...
        let src = [0xef, 0xa3, 0x96, 0xef, 0xa3, 0x94, 0xef, 0xa3, 0x95, 0xef, 0xa3, 0x99, 0xef, 0xa3, 0xa5, 0xef, 0xa3, 0xa9, 0xef, 0xa3, 0x9a,
                   0xef, 0xa3, 0x94, 0xef, 0xa3, 0x96, 0x20, 0xef, 0xa3, 0xa0, 0xef, 0xa3, 0x90, 0xef, 0xa3, 0xa0, 0x20, 0xef, 0xa3, 0x98, 0xef,
                   0xa3, 0x90, 0xef, 0xa3, 0x98, 0xef, 0xa3, 0xa6, 0xef, 0xa3, 0x90, 0xef, 0xa3, 0x9a, 0xef, 0xa3, 0xbe];
        ...
        let src = [0xf0, 0x9f, 0x98, 0x81, 0xf0, 0x9f, 0x98, 0x82, 0xf0, 0x9f, 0x98, 0x83, 0xf0, 0x9f, 0x98, 0x84];
