use book::volume::Volume;
use book::word::Word;
use book::extractor::traits::FromPointer;
...
impl<'a> Pointer<'a> {
    ...
    pub fn unalias (self) -> Pointer<'a> {
        match self.vol.gist[self.pos] {
            (_, _, Word::Alias (sz)) => Pointer::unalias (Pointer { vol: self.vol, pos: sz }),
            _ => self
        }
    }
    ...
    pub fn next_sibling (self) -> Option<Pointer<'a>> {
        ...
        for i in (self.pos+1) .. self.vol.gist.len () {
            ...
            if sub == level { return Some (Pointer { vol: self.vol, pos: i }) }
            if sub < level { break };
        }
    ...
    pub fn into_seq (self) -> Option<Pointer<'a>> {
        ...
        match *word {
            Word::Seq (_) => if let Some (&(_, sublevel, _)) = self.vol.gist.get (ptr.pos + 1) {
                if sublevel == level + 1 {
                    Some ( Pointer { vol: ptr.vol, pos: ptr.pos + 1 } )
                } else { None }
            } else { None },
            _ => None
        }
    ...
    pub fn into_map (self) -> Option<Pointer<'a>> {
        ...
        match *word {
            Word::Map (_) => if let Some (&(_, sublevel, _)) = self.vol.gist.get (ptr.pos + 1) {
                if sublevel == level + 1 {
                    Some ( Pointer { vol: ptr.vol, pos: ptr.pos + 1 } )
                } else { None }
            } else { None },
