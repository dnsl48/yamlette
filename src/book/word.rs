        ...
        match self {
            ...
            Word::Scalar (tagged_value) => {
                if let Some (_) = tagged_value.as_any ().downcast_ref::<String> () {
                    ...
                    return Ok (*s.clone ())
                } else { Err (Word::Scalar (tagged_value)) }
            }
            _ => Err (self)
        }
    }
    ...
    fn into (self) -> Result<&'a BigInt, &'a Word> {
        match *self {
            Word::Int (ref val) => {
                ...
                if let Some (v) = maybe { Ok (v) } else { Err (self) }
            }
            Word::Scalar (ref tagged_value) => {
                if let Some (val) = tagged_value.as_any ().downcast_ref::<BigInt> () {
                    Ok (val)
...
impl Into<Result<u8, Word>> for Word {
    fn into (self) -> Result<u8, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_u8 () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_u8 () { return Ok (i) }
            },
...
impl Into<Result<i8, Word>> for Word {
    fn into (self) -> Result<i8, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_i8 () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_i8 () { return Ok (i) }
            },
...
impl Into<Result<u16, Word>> for Word {
    fn into (self) -> Result<u16, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_u16 () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_u16 () { return Ok (i) }
            },
...
impl Into<Result<i16, Word>> for Word {
    fn into (self) -> Result<i16, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_i16 () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_i16 () { return Ok (i) }
            },
...
impl Into<Result<u32, Word>> for Word {
    fn into (self) -> Result<u32, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_u32 () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_u32 () { return Ok (i) }
            },
...
impl Into<Result<i32, Word>> for Word {
    fn into (self) -> Result<i32, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_i32 () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_i32 () { return Ok (i) }
            },
...
impl Into<Result<u64, Word>> for Word {
    fn into (self) -> Result<u64, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_u64 () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_u64 () { return Ok (i) }
            },
...
impl Into<Result<i64, Word>> for Word {
    fn into (self) -> Result<i64, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_i64 () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_i64 () { return Ok (i) }
            },
...
impl Into<Result<usize, Word>> for Word {
    fn into (self) -> Result<usize, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_usize () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_usize () { return Ok (i) }
            },
...
impl Into<Result<isize, Word>> for Word {
    fn into (self) -> Result<isize, Word> {
        match self {
            Word::Int (ref val) => {
                if let Some (i) = val.to_isize () { return Ok (i) }
            },
            Word::Float (ref val) => {
                if let Some (i) = val.to_isize () { return Ok (i) }
            },
