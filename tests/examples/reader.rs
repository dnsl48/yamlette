...
macro_rules! read {
    ($src:expr) => {{
        ...
        reader
            .read(SliceReader::new($src.as_bytes()), &mut |block| {
                if let Err(_) = sender.send(block) {
                    Err(Cow::from("Cannot yield a block"))
                } else {
                    Ok(())
                }
            })
            .unwrap_or_else(|err| {
                assert!(false, "Unexpected result: {}, :{}", err, err.position);
            });
...
macro_rules! read_bytes {
    ($src:expr) => {{
        ...
        reader
            .read(SliceReader::new($src), &mut |block| {
                if let Err(_) = sender.send(block) {
                    Err(Cow::from("Cannot yield a block"))
                } else {
                    Ok(())
...
macro_rules! the_end {
    ( $receiver:expr ) => {
        if let Ok(block) = $receiver.try_recv() {
            ...
            if let BlockType::StreamEnd = block.cargo {
            } else {
                assert!(
                    false,
                    "Unexpected result: expected StreamEnd, got {:?}",
                    block.cargo
                )
            }
        } else {
            assert!(false, "Cannot fetch the last block")
        }
    };
}
...
macro_rules! expect {
    ...
    ( $receiver:expr, ( $level:expr, $parent:expr, $index:expr ), dir, yaml, $version:expr ) => {{
        if let Ok (block) = $receiver.try_recv () {
            ...
            } else { assert! (false, "Unexpected result: expected DirectiveYaml, got {:?}", block.cargo) }
    ...
    ( $receiver:expr, ( $level:expr, $parent:expr, $index:expr ), $data:expr, dir, tag, $handle:expr, $prefix:expr ) => {{
        if let Ok (block) = $receiver.try_recv () {
            ...
            } else { assert! (false, "Unexpected result: expected DirectiveTag, got {:?}", block.cargo) }
        } else { assert! (false, "Cannot fetch a new block") }
    }};
    ...
    ( $receiver:expr, ( $level:expr, $parent:expr, $index:expr ), doc, start ) => {{
        if let Ok (block) = $receiver.try_recv () {
            ...
            if let BlockType::DocStart = block.cargo { } else { assert! (false, "Unexpected result: expected DocStart, got {:?}", block.cargo) }
        } else { assert! (false, "Cannot fetch a new block") }
    }};
    ...
    ( $receiver:expr, ( $level:expr, $parent:expr, $index:expr ), $data:expr, node, mapping ) => {{
        if let Ok (block) = $receiver.try_recv () {
            ...
            if let BlockType::Node (node) = block.cargo {
                ...
                if let NodeKind::Mapping = node.content { } else { assert! (false, "Unexpected result {:?}", node); }
    ...
    ( $receiver:expr, ( $level:expr, $parent:expr, $index:expr ), node, sequence ) => {{
        if let Ok (block) = $receiver.try_recv () {
            ...
            if let BlockType::Node (node) = block.cargo {
                assert! (node.anchor.is_none ());
                assert! (node.tag.is_none ());
                if let NodeKind::Sequence = node.content { } else { assert! (false, "Unexpected node {:?}", node); }
            } else { assert! (false, "Unexpected block {:?}", block) }
        } else { assert! (false, "Unexpected result") }
    }};
    ...
    ( $receiver:expr, ( $level:expr, $parent:expr, $index:expr ), $data:expr, node, scalar, $val:expr ) => {{
        if let Ok (block) = $receiver.try_recv () {
            ...
            } else { assert! (false, "Unexpected result / not a node / {:?}", block.cargo) }
...
- [Sammy Sosa  , 63, 0.288]";
    ...
    #[test]
    fn example_02_06() {
        let src = r"Mark McGwire: {hr: 65, avg: 0.278}
Sammy Sosa: {
    hr: 63,
    avg: 0.288
  }";
    ...
    #[test]
    fn example_02_07() {
        let src = r"# Ranking of 1998 home runs
---
- Mark McGwire
- Sammy Sosa
- Ken Griffey
...
stats: |
  ...
  0.278 Batting Average";
    ...
    #[test]
    fn example_02_17() {
        let src = r##"unicode: "Sosa did fine.\u263A"
...
hexadecimal: 0xC";
    ...
    #[test]
    fn example_02_20() {
        let src = r"canonical: 1.23015e+3
...
string: '012345'";
    ...
    #[test]
    fn example_02_22() {
        let src = r"canonical: 2001-12-15T02:59:43.1Z
...
";
    ...
    fn example_02_24() {
        ...
        let src = r"%TAG ! tag:clarkevans.com,2002:
--- !shape
...
- !label
  ...
  text: Pretty vector drawing.";
    ...
    #[test]
    fn example_02_25() {
        let src = r"# Sets are represented as a
...
? Ken Griff";
    ...
    #[test]
    fn example_02_26() {
        let src = r"# Ordered maps are represented as
...
- Ken Griffy: 58";
    ...
    #[test]
    fn example_02_27() {
        let src = r"--- !<tag:clarkevans.com,2002:invoice>
invoice: 34843
date   : 2001-01-23
bill-to: &id001
    given  : Chris
...
"#;
    ...
    #[test]
    fn example_set_1() {
        let src = r"baseball players: !!set
  ? Mark McGwire
  ? Sammy Sosa
  ? Ken Griffey
# Flow style
...
link with:
  ...
  - = : library2.dll
    ...
    #[test]
    fn example_05_01() {
        let src = b"\xEF\xBB\xBF# Comment only.";
...
mapping: { sky: blue, sea: green }";
    ...
    #[test]
    fn example_05_06() {
        let src = "anchored: !local &anchor value
alias: *anchor";
...
!!str "text""#;
    ...
    #[test]
    fn example_05_10() {
        let src = "commercial-at: @text
grave-accent: `text";

        let receiver = read_with_error!(
            src,
            r"@ character is reserved and may not be used to start a plain scalar",
            15
        );
        let mut data = data!();
...
A A A""#;
  ...
  \xq-""#;
    ...
    #[test]
    fn example_06_01() {
        let src = r#"  # Leading comment line spaces are
...
"#;
    ...
    #[test]
    fn example_06_12() {
        let src = r#"{ first: Sammy, last: Sosa }:
# Statistics:
  hr:  # Home runs
     65
  avg: # Average
...
!!map {
  ...
  : !!map {
    ...
    #[test]
    fn example_06_13() {
        let src = r#"%FOO  bar baz # Should be ignored
               # with a warning.
--- "foo""#;
        ...
        expect!(
            ...
            (0, 0, 1),
            warning,
            "Unknown directive at the line 0",
            0
        );
        expect!(receiver, (0, 0, 2), doc, start);
        expect!(receiver, (0, 0, 0), datum, data);
    ...
    #[test]
    fn example_06_14() {
        let src = r#"%YAML 1.3 # Attempt parsing
           # with a warning
---
"foo""#;
        ...
        expect!(
            ...
            (0, 0, 1),
            warning,
            "%YAML minor version is not fully supported",
            10
        );
...
foo"#;
        ...
        let receiver = read_with_error!(
            src,
            r"The YAML directive must only be given at most once per document",
            10
        );
        let mut data = data!();
    ...
    #[test]
    fn example_06_16() {
        let src = r#"%TAG !yaml! tag:yaml.org,2002:
---
!yaml!str "foo""#;
...
!<tag:example.com,2000:app/foo> "bar""#;
    ...
    #[test]
    fn example_06_19() {
        let src = r#"%TAG !! tag:example.com,2000:app/
---
...
"#;
    ...
    #[test]
    fn example_06_20() {
        let src = r#"%TAG !e! tag:example.com,2000:app/
---
!e!foo "bar""#;
...
!!map {
  ...
  : *B1,
    ...
    #[test]
    fn example_06_24() {
        let src = r#"!<tag:yaml.org,2002:str> foo :
  !<!bar> baz"#;
...
!!seq [
  ...
  !<tag:example.com,2000:app/tag!> "baz"
    ...
    #[test]
    fn example_06_27() {
        let src = r#"%TAG !e! tag:example,2000:app/
---
- !e! foo
- !h!bar baz"#;
    ...
    #[test]
    fn example_06_28() {
        let src = r#"# Assuming conventional resolution:
- "12"
- 12
- ! 12"#;
...
!!seq [
  ...
  !<tag:yaml.org,2002:str> "12",
    ...
    #[test]
    fn example_06_29() {
        let src = r#"First occurrence: &anchor Value
...
!!map {
  ...
  : *A,
    ...
    #[test]
    fn example_07_01() {
        let src = r#"First occurrence: &anchor Foo
Second occurrence: *anchor
...
!!str "here's to \"quotes\"""#;
 ...
 2nd non-empty 
    ...
    #[test]
    fn example_07_10() {
        let src = r#"# Outside flow collection:
- ::vector
- ": - ()"
- Up, up, and away!
- -123
...
!!seq [
 ...
 "foo  bar": invalid ]"#;
        ...
        expect!(
            ...
            node,
            scalar,
            r#""foo  bar""#
        );
        expect!(receiver, (1, 2, 10), data, block, map, (1, 2, 9));
        expect!(receiver, (2, 10, 11), data, node, scalar, r"invalid");
        expect!(receiver, (0, 0, 2), node, sequence);
...
"#;
    ...
    #[test]
    fn example_08_17() {
        let src = r#"? explicit key # Empty value
? |
  block key
...
"#;
    ...
    #[test]
    fn example_08_18() {
        let src = r#"plain key: in-line value
: # Both empty
"quoted key":
- entry
"#;
...
%!PS-Adobe-2.0 # Not the first line"#;
        ...
        expect!(
            ...
            data,
            literal,
            r"%!PS-Adobe-2.0 # Not the first line"
        );
        expect!(receiver, (0, 0, 2), node, block, close);
        expect!(receiver, (0, 0, 4), doc, end);
...
Flow style: !!str "String: just a theory.""#;
    ...
    #[test]
    fn example_10_04() {
        let src = r#"!!null null: value for null key
key with null value: !!null null"#;
    ...
    #[test]
    fn example_10_05() {
        let src = r#"YAML is a superset of JSON: !!bool true
